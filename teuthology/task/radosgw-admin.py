...
def rgwadmin(ctx, client, cmd, raw = False):
...
def task(ctx, config):
    ...
    (err, out) = rgwadmin(ctx, client, ['policy', '--bucket', bucket.name, '--object', key.key], raw=True)

    assert out

    policy = ET.fromstring(out)
    acls = policy.find('{http://s3.amazonaws.com/doc/2006-03-01/}AccessControlList').findall('{http://s3.amazonaws.com/doc/2006-03-01/}Grant')
    assert len(acls) == 1
